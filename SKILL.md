...
2. Is it a `@MainActor` class? → Usually implicitly `Sendable`, but check superclass — SE-0434 exception for subclasses of nonisolated non-Sendable types
3. Can region-based isolation prove the usage is safe? → Check if `sending` keyword helps. See [isolation-patterns.md](references/isolation-patterns.md)
4. Is it a class with internal locking? → `@unchecked Sendable`
5. Is the error from an API you don't control? → `@preconcurrency import` (see [migration-guide.md](references/migration-guide.md))
